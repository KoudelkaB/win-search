...
Seit dem Laden aus NTFS geänderte Elemente zeigen immer die aktuelle Zeit unter **Geändert** und die aktuelle **Größe**, auch während die schreibende Anwendung die Datei noch geöffnet hält – das macht das Überwachen, wohin eine Anwendung schreibt, verlässlich. Der Explorer kann für dieselbe Datei ältere Werte anzeigen: NTFS veröffentlicht sie erst etwa eine Sekunde nach einem Schreibvorgang im Verzeichnis oder erst beim Schließen der Datei, was bei einem Ereignisprotokoll oder einer Datenbank Tage dauern kann. `F12` lädt alles erneut aus NTFS, sodass solche Elemente auf die veröffentlichten Werte zurückfallen, bis sie sich erneut ändern.
