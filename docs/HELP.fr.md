...
- `A` : changer la date de dernier accès, avec les mêmes choix `V` et `C`.
- Ajoutez `O` en premier pour remplacer les cibles existantes lorsque cela est pris en charge.

Les renommages et les changements de dates sur des éléments protégés en écriture (tout ce qui se trouve sous `Program Files` ou `Windows`, par exemple) sont automatiquement réessayés via l’assistant élevé. Si celui-ci n’a pas encore été lancé, c’est à ce moment que sa demande UAC apparaît.
...
Les éléments modifiés depuis le chargement NTFS affichent toujours l’heure actuelle dans **Modifié** et la **Taille** actuelle, même pendant que l’application qui écrit garde le fichier ouvert — c’est ce qui rend fiable l’observation de l’endroit où une application écrit. L’Explorateur peut afficher des valeurs plus anciennes pour le même fichier : NTFS ne les publie dans le répertoire qu’environ une seconde après une écriture, ou seulement à la fermeture du fichier, ce qui pour un journal d’événements ou une base de données peut prendre des jours. `F12` recharge tout depuis NTFS : ces éléments reviennent donc aux valeurs publiées jusqu’à leur prochaine modification.
