...
- The `installservice` task is selected by default, so winget installs get the service even though a silent install can never show the checkbox. On an existing install `InitializeWizard` presets the task from whether the service actually exists (not from `UsePreviousTasks`), so a `winget upgrade` keeps what the machine already has: an install that declined the service keeps declining it, and one with the service keeps it. Test a silent upgrade of both kinds before release.
